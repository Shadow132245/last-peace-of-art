        ...
        <div className="mt-4 flex items-center gap-3 text-sm text-zinc-500">
          <Link href={`/user/${encodeURIComponent(project.user.name)}`} className="flex items-center gap-2 hover:text-zinc-900 dark:hover:text-zinc-100">
            ...
            {(() => {
              ...
              const userRoles = ((project.user.roles as string[]) ?? []).filter((r) => r !== "user");
              const displayRoles = userRoles.length ? userRoles : (project.user.role !== "user" ? [project.user.role] : []);
              return displayRoles.map((r) => (
                <span key={r} className={`rounded px-1.5 py-0.5 text-[10px] font-semibold ${roleStyles[r] ?? ""}`}>{r.toUpperCase()}</span>
              ));
            })()}
          </Link>
          <span>&middot;</span>
