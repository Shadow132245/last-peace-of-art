...
export default async function SuspendedPage() {
  ...
  if (!session) {
    return (
      <div className="mx-auto flex min-h-[60vh] max-w-lg flex-col items-center justify-center px-4 text-center">
        ...
        <h1 className="mb-3 text-3xl font-bold">{t("suspended.title")}</h1>
        <p className="mb-6 text-zinc-500">{t("suspended.noSession")}</p>
  ...
  if (!user?.suspended) {
    return (
      <div className="mx-auto flex min-h-[60vh] max-w-lg flex-col items-center justify-center px-4 text-center">
        <div className="mb-6 text-6xl">✅</div>
        <h1 className="mb-3 text-3xl font-bold">{t("suspended.activeTitle")}</h1>
        <p className="mb-6 text-zinc-500">{t("suspended.activeDesc")}</p>
    ...
    <div className="mx-auto flex min-h-[60vh] max-w-lg flex-col items-center justify-center px-4 text-center">
      ...
      <FadeInView delay={0.2}>
        <div className="mb-8 w-full rounded-2xl border border-red-200/80 bg-red-50/50 p-6 dark:border-red-900/50 dark:bg-red-950/30">
          <p className="mb-3 text-sm font-medium text-red-700 dark:text-red-300">{t("suspended.reasonLabel")}</p>
          <p className="mb-4 text-zinc-700 dark:text-zinc-300">{user.suspensionReason || t("suspended.noReason")}</p>
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            {isPermanent ? t("suspended.permanent") : t("suspended.temporary").replace("{date}", untilDate ?? "")}
          </p>
        </div>
      </FadeInView>
      <FadeInView delay={0.3}>
        <p className="mb-6 text-zinc-500">
          {t("suspended.appealText")}
        </p>
      </FadeInView>
