...
export function ApplicationActions({ applicationId }: { applicationId: string }) {
  ...
  const handleAction = async (status: "approved" | "rejected") => {
    ...
    await fetch(`/api/admin/applications/${applicationId}`, {
      ...
      body: JSON.stringify({
        status,
        adminResponse: status === "approved" ? "Congratulations! Your application has been approved. Welcome to the team!" : "Thank you for applying, but we have decided to go with other candidates at this time.",
      }),
    });
    router.refresh();
    setLoading(null);
